...
export function generarHtmlComprobante(venta: VentaConDetalle, resultadoCAE?: ResultadoCAE) {
  ...
  const filasItems = venta.items
    .map(
      ...
      <tr>
        ...
        <td class="num">${formatearMoneda(item.precioUnitario)}</td>
        <td class="num">${formatearMoneda(item.subtotal)}</td>
      </tr>`
    )
    .join("");
...
<body onload="window.print()">
  ${
    esMock
      ? `<div class="aviso-mock">Comprobante generado en modo de prueba (ARCA_MODO=mock). El CAE es simulado y <strong>no tiene validez fiscal</strong>.</div>`
      : ""
  }
  ...
  <table>
    <thead>
      <tr><th>Producto</th><th class="num">Cant.</th><th class="num">P. Unit.</th><th class="num">Subtotal</th></tr>
    </thead>
    <tbody>
      ${filasItems}
    </tbody>
